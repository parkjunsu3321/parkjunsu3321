# 👋 Hi, I'm 박준수

> Agentic AI Developer를 목표로 성장하고 있습니다.

---

## 🚀 About Me

* 🤖 Agent 기반 AI 시스템 설계에 관심
* 🧠 LLM / RAG / Knowledge Graph 연구 및 구현
* 🔄 LangChain & LangGraph 기반 워크플로우 구축
* 📊 데이터 분석 & 머신러닝 프로젝트 경험
* 🎯 목표: 문제 해결 중심의 Agentic AI 엔지니어

---

## 🧠 Tech Stack

### 💻 Language

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge\&logo=python\&logoColor=white)

### 📊 Data & AI

![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge\&logo=pandas)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=for-the-badge\&logo=scikit-learn)
![XGBoost](https://img.shields.io/badge/XGBoost-EC6F00?style=for-the-badge)
![LightGBM](https://img.shields.io/badge/LightGBM-02569B?style=for-the-badge)

### ⚙️ Tools (Agentic AI)

![LangChain](https://img.shields.io/badge/LangChain-000000?style=for-the-badge)
![LangGraph](https://img.shields.io/badge/LangGraph-4B8BBE?style=for-the-badge)

---

## 📌 Projects

### 🔹 AI / Data Projects

* 🧪 분자 특성 예측 (SMILES 기반 ML 모델링)
* 📈 데이터 분석 파이프라인 구축 (Pandas + 시각화)

### 🔹 Agentic AI / LLM Projects

* 🧠 RAG / CAG / TAG 구조 분석 및 구현
* 🔄 LangGraph 기반 Agent Workflow 설계
* 📚 법령 데이터 기반 Knowledge Graph 구축

---

## 📊 GitHub Stats


![GitHub stats](https://github-readme-stats.vercel.app/api?username=parkjunsu3321&show_icons=true&count_private=true)

![GitHub Streak](https://streak-stats.demolab.com?user=parkjunsu3321)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=parkjunsu3321&layout=compact)

---

## 📫 Contact

* GitHub: [https://github.com/your-username](https://github.com/your-username)
* Email: [arkob3321@gmail.com](mailto:arkob3321@gmail.com)

---

## 💡 Motto

> "Agent는 단순한 자동화가 아니라, 사고하는 시스템이다."
